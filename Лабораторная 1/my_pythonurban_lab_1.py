# -*- coding: utf-8 -*-
"""My PythonUrban_lab_1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1UC_i6pfDzhhUfBx_ws354n8ysJHKRDrM
...
def task_1(list_cities):
  ...
  for item in list_cities:
    cities_capitalized.append(str.capitalize(item))
  return cities_capitalized
...
def task_2(list_dict_cities, filter_population):
  new_list_dict_cities = []
  for item in list_dict_cities:
    if item.get("population") > filter_population:
      new_list_dict_cities.append(item)
...
def task_3(first_list, second_list):
  first_set = set(first_list)
  second_set = set(second_list)
  return first_set.intersection(second_set)

common_cities = sorted(task_3(first_group, second_group))  # TODO вызвать функцию и передать в неё значения fisrt_group и second_group
...
def green_index_calc(list_territories):
  list_green_indices = []
  for item in list_territories:
    green_zones_area = 0
    for zone in item.get("green_zones"):
      green_zones_area += zone
    green_index = green_zones_area / item.get("territory_area")
    list_green_indices.append({"territory_name" : item.get('Name'), "green_index" : green_index})
  return list_green_indices

# TODO написать функцию, которая будет высчитывать средний индекс озеленения по всем территориям
def green_index_average(list_green_indices):
  green_ind_sum = 0
  for item in list_green_indices:
    green_ind_sum += item.get("green_index")
  return green_ind_sum / len(list_green_indices)

# TODO распечатать результат округленный до 4 знаков после запятой
print(round(green_index_average(green_index_calc(list_territories)), 4))